...
var sliderFactory = {
	createSlider: function() {
		var newSlider =  new Slider();
			...
			imgesArray: [], // создаем массив пустой
			currentImagesIdnex: 0, // переменная со значением 0
			
			start: function(elId){ // функция внутри объекта, которая запускает дейстаия слайдера
			...
			onShowPrevBtnClick: function(event) { // создаем функцию переключения Назад		
			...
			onShowNextBtnClick: function(event) { // создаем функцию переключения Вперёд
